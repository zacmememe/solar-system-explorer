...
**物理位置（2026-09-23 迁移）**：项目目录与 Google Drive 同步，重资产已全部迁至 `D:\solar-evidence\`（`pro-review\` 证据工件、`data-cache\` 原始 DTM 缓存、`review-exports-archive\` 历史导出包）。本文及历史交接文档中的 `artifacts/pro-review/…`、`/data/…` 路径指同一批文件的历史位置；验证脚本输出默认已指向 `D:/solar-evidence/pro-review`（EVIDENCE_DIR/DEM_CACHE_DIR 环境变量可覆盖）。运行环境：本地 `vite preview`（dist）+ Edge headless 1440×900。
...
| 6 | `86-p2-descent-demo.mp4`（H.264 4.9MB，另有原始 `86-p2-descent-demo-mjpeg.avi` 79.4MB）+ `batch-p2-descent-demo-fingerprint.json` | 代表录像 78.6s@16fps：启动→25/50/75% 暂停→触地（MSL −1689.2，admission admitted）→返轨；0 控制台错误。指纹含运行时状态（相机模式/网格/帧计数/里程碑遥测）。MP4 由便携 ffmpeg 9（`D:\Apps\ffmpeg`）转码 | `record-p2-descent-demo.ts`（753d1ff…） |
...
3. 录像无音频；MP4 为分发版（H.264 yuv420p faststart），AVI 为零依赖原始版，内容同源。
