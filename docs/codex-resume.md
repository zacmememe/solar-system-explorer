...
2. [最新阶段HUD报告](phase-aware-hud-report.md)：观星/下降/悬停/地表/返程统一仪表与全景书签；[下一步盖尔网格方案](plans/gale-display-ground-refinement.md)保留地形只读诊断。前轮 [火星与天体真实感报告](surface-atmosphere-realism-report.md)包含火星大气/地面/夜间遮光、3张新观测卫星图和30体后续清单。导航上下文见 [导航与视觉报告](navigation-experience-report.md)：跨星避障、相机连续性、卫星母星构图、土星环影。地形上下文保留于 [月面连续性报告](lunar-boundary-continuity-report.md)，更早系统修复见 [系统报告](codex-system-polish-report.md)。
3. [体验基础契约](experience-foundations.md)：物理空间/相机/时间/存档约束与真实 UI 验证方法。

最新产品候选为 `7c6bb5f93d673bd3252f25adc36806eadbf31ce2`（本轮base `e0521091793d7b518b15836453f4c5e9da4fcc0f`），生产包 `index-DiTLfhqA.js` / CSS `index-BHK1GWJu.css`；SHA-256、验证边界与后续任务见最新HUD报告。334/334测试、最终月火两站39帧正常UI和地形故障退出通过。旧报告1353cf5及更早导航/月面修复已包含在历史中。如 HEAD 之后只有证据与文档提交，不要误称产品漂移；如 src、资产、配置变动，则按新候选重验。核对 `git status`，其他作者原有未跟踪文件不能删除或一并提交。
...
最新优先事项是按盖尔方案做单站近地网格/净空的同机位诊断，再局部细化；目前只有只读证据，没有实施这项几何修改。HUD眼高与可见网格净空不自动等价。之后再按前轮清单替换火卫一、土卫七的明显程序形状，以及天卫五/天卫一/其余伽利略卫星的程序图；先做资产/许可/坐标与缺测调查，不扩张站点。木卫一、土卫一、土卫二已接官方图，勿重复替换或把增强色称作自然色。月面剩余问题继续区分实际网格抽稀折面、NAC↔全球图的低频色调差与 2–5m 影像上限。每条边先明确双方的网格/材质和实际机位，勿重复换材质或泛调透明度。严禁移动天体、抬高真实地形、关深度掩盖问题。新过渡圈只用于静海，Taurus WAC 仍有 offset/repeat 语义，不能未经核对批量套用。
...
- `npm test`：完整单元/契约门禁。
- `npm run verify:hud`：默认耶泽罗；`SITE_ID=tranquility-base`改查静海，包含全景书签、取消、桌面/横竖屏说明卡、地表指南针和返回。`06-preparing`可能在截图时已进入下降，不能按文件名虚构准备期视觉验收。
