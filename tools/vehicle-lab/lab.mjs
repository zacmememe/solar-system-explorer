...
import { profiles, prepareModel, setPresentationSpan, fitPresentationToViewport, assembleShuttle, refineMaterials, resourcesOf } from './model.mjs';
...
async function load() {
  ...
  try {
    let raw;
    if (id === 'iss') raw = VehicleMeshBuilder.buildVehicle('iss');
    ...
    changes = ['refined', 'materials-only'].includes(variant) && id !== 'iss' ? refineMaterials(raw, id, renderer.capabilities.getMaxAnisotropy()) : [];
    try { current = prepareModel(raw, profiles[id]); } catch (e) { release(); throw e; }
...
window.vehicleLab = { snapshot, ready: () => loadState === 'ready', raw: () => current, select: async settings => {
  for (const key of ['vehicle', 'variant', 'view', 'lighting', 'sun']) if (settings[key]) $('#' + key).value = settings[key];
  angles = settings.angle || 'front'; await load(); return snapshot();
} };
resize(); await load();
