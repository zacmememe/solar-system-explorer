...
const report = { base: '9f37d19d80c66ef59977bf74183571345fef8757', baseline, indexSha256: sha(await fs.readFile('dist/index.html')), cases: [], errors: [], expectedNetworkErrors: [] };
...
const sourceGlb = await fs.readFile('public/assets/models/hubble-nasa-b.glb');
const jsonLength = sourceGlb.readUInt32LE(12), gltf = JSON.parse(sourceGlb.subarray(20, 20 + jsonLength).toString());
// This NASA Hubble asset has no textures. Add one missing dependency to a used
// material in the intercepted response only; never alter the checked-in asset.
gltf.images = [{ uri: '/injected-missing-vehicle-texture.png' }];
...
try {
  ...
  page.on('request', req => {
    ...
    if (!req.url().includes('hubble-nasa-b.glb')) { void req.continue(); return; }
    requests++;
  ...
  await click('toolbar-hangar-btn'); await click('hangar-vehicle-item-hubble'); await delay(2000);
  ...
  if (baseline) {
    ...
    assert.equal(boarded.selected, 'hubble'); assert.equal(boarded.model.isGlb, false);
    ...
    assert.equal(failedMain.selected, null); assert.equal(failedMain.visible, false); assert.match(failedMain.text, /未能加载/); assert.doesNotMatch(failedMain.text, /已登船/);
    fault = 'delay'; await click('toolbar-hangar-btn'); await click('hangar-vehicle-item-hubble'); await waitPending();
    ...
    await click('hangar-vehicle-item-hubble'); await waitPending(); await click('hangar-vehicle-item-iss');
    await ready();
    ...
    fault = 'none'; await click('toolbar-hangar-btn'); await click('hangar-vehicle-item-hubble');
    await ready();
    fault = 'delay'; await click('hangar-board-btn'); await waitPending();
    const loadingMain = await shot('08-main-pending'); assert.equal(loadingMain.selected, null); assert.doesNotMatch(loadingMain.text, /已登船/);
    ...
    await click('toolbar-hangar-btn'); await click('hangar-vehicle-item-hubble'); await ready(); await click('hangar-board-btn');
    await wait(() => window.__solarEngine.getCurrentVehicle() === 'hubble' && window.__solarEngine.vehicleGroup.visible);
    const hubble = await shot('09-hubble-formation'); assert.equal(hubble.model.isGlb, true); assert.match(hubble.text, /已登船/);
