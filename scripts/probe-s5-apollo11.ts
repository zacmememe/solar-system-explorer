/**
 * S5-3 探针：月面多站点化——静海基地（Apollo 11）下降流
...
async function main() {
  ...
  await flyToSiteZenith(0.6741, 23.4730);
  await page.waitForFunction(() => {
    const s = (window as any).__solarEngine.cameraController.getSnapshot();
    return !s.isTransitioning;
  ...
  const flyDiag = await page.evaluate(`(() => {
    ...
    return {
      ...
      dotHadley: +dot(0.6741, 23.4730).toFixed(4),
  ...
  if (avail.siteId !== 'tranquility-base') throw new Error(`站点解析错误: ${avail.siteId}`);
  ...
  await page.waitForFunction(() => {
    const e = (window as any).__solarEngine;
    return !!e.getMoonMesh?.()?.getObjectByName?.('tranquility-base-terrain')?.visible;
  ...
  const stacks = await page.evaluate(`(() => {
    const e = window.__solarEngine;
    const h = e.getMoonMesh().getObjectByName('moon-site-tranquility-base');
  ...
  const priv = await page.evaluate(`(() => {
    const e = window.__solarEngine;
    const h = e.moonSiteStacks.get('tranquility-base');
    const t = e.moonSiteStacks.get('taurus-littrow');
    const hp = e.terrainProviderProbe;
    return {
      engineActiveMoonSite: e.activeMoonSiteId,
      siteSourceTag: h ? h.raster.sourceTag : null,
      siteReady: h ? h.raster.isReady : null,
      taurusSourceTag: t ? t.raster.sourceTag : null,
      siteGroupVisible: h ? h.group.visible : null,
  ...
  const hpDiag = await page.evaluate(`(async () => {
    ...
    return {
      ...
      ctlSampleAtSite: ctlHp.getHeightSample('moon', 0.6741, 23.4730),
      dynamicImportUrl: mod[Symbol.toStringTag],
    };
  })()`);
  console.log('[6b] provider internals =', JSON.stringify(hpDiag, null, 1));
  if (tele.site !== 'tranquility-base') throw new Error(`telemetry site mismatch: ${tele.site}`);
  if (tele.terrain.sourceId !== 'NAC_DTM_APOLLO11') throw new Error(`sourceId mismatch: ${tele.terrain.sourceId}`);
  ...
  const glShot = (name: string) =>
    page.evaluate(`(() => {
      ...
      fs.writeFileSync(`${OUT}/s54-${name}.png`, Buffer.from(res.b64.split(',')[1], 'base64'));
