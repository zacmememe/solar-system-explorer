/**
 * Mars Deep Visual Review & Verification Script
 * 1. Fly to Mars and verify camera framing & surface lighting
 * 2. Verify Lommel-Seeliger mineral diffuse, polar ice cap albedo & blue twilight
 * 3. Verify Mars details panel and scientific metrics
...
async function main() {
  console.log('===============================================================');
  console.log('🔴 太阳系漫游 · 红色星球【火星系统】深度视觉与着色器实机审视');
  console.log(`目标 URL: ${TEST_URL}`);
  console.log(`执行浏览器: ${executablePath}`);
  console.log('===============================================================\n');

  const userDataDir = path.join(os.tmpdir(), `solar_mars_${Date.now()}_${Math.random().toString(36).slice(2)}`);
  ...
  try {
    ...
    if (hudTargetText !== '火星' || hudRadiusText !== '3,390') {
      throw new Error(`HUD 数据异常: target=${hudTargetText}, radius=${hudRadiusText}`);
    }

    // 截图 1：火星饱满构图、红砂矿物漫反射、极地冰盖与前向晨昏微蓝
    const shot1 = path.join(OUT_DIR, '07-mars-framed.png');
    await page.screenshot({ path: shot1 });
    console.log(`✅ [截帧 1] 火星岩石矿物漫反射与极地冰盖构图已保存: ${shot1}`);

    // 打开了解这颗天体资料面板
    console.log('2. 正在打开火星深入科学资料面板...');
    await page.click('[aria-controls="hud-details-panel"]');
    await pause(400);

    const shot2 = path.join(OUT_DIR, '08-mars-details-panel.png');
    await page.screenshot({ path: shot2 });
    console.log(`✅ [截帧 2] 火星资料面板与科学参数已保存: ${shot2}`);
    ...
    // 打开观测选项并切换暗部补光
    console.log('3. 正在测试教学补光 (teachingLight) 对火星背阳面奥林匹斯山/峡谷地貌的提亮...');
    ...
    const shot3 = path.join(OUT_DIR, '09-mars-teaching-light.png');
    await page.screenshot({ path: shot3 });
    console.log(`✅ [截帧 3] 火星教学暗部补光对比图已保存: ${shot3}`);
    ...
    // 4. 性能与帧率测定
    console.log('4. 正在采样火星场景下 3D 渲染循环的真实帧时间 (采样 150 帧)...');
    await page.evaluate(() => {
