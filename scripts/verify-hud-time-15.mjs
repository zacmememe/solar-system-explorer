...
try{
 ...
 if(!process.env.BASELINE){
  ...
  for(const width of [1920,1440,1024,390]){
   ...
   const layout=await page.evaluate(()=>{const h=document.querySelector('.hud-layout').getBoundingClientRect();const r=document.querySelector('.hud-context').getBoundingClientRect();const controls=[...document.querySelectorAll('.hud-layout button,.hud-layout select')].map(e=>{const b=e.getBoundingClientRect(),hit=document.elementFromPoint(b.x+b.width/2,b.y+b.height/2);return {label:e.textContent,within:b.x>=0&&b.right<=innerWidth,hit:hit===e||e.contains(hit),overlapsMap:b.x<r.right&&b.right>r.x&&b.y<r.bottom&&b.bottom>r.y};});return {height:h.height,controls};});
   report.samples.push({label:'moon-layout-'+width,...layout});assert.ok(layout.controls.every(c=>c.within&&c.hit&&!c.overlapsMap),'moon controls must be reachable without overlapping the map: '+width);if(width>=1024)assert.ok(layout.height<=110);await shot('moon-'+width);
  }
