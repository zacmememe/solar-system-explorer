/**
 * S5-2 测试：Apollo 15 数据链（哈德利月溪）——apollo15-v1 NAC DTM 包 +
 * lola-l1-hadley-v1 LOLA L1 窗包的完整性、注册与独立交叉校验。
...
const DIR = path.resolve('public/data/dem/apollo15-v1');
const LDIR = path.resolve('public/data/dem/lola-l1-hadley-v1');
...
describe('S5-2：apollo15-v1 NAC DTM 数据包', () => {
  it('元数据完整：APOLLO15 DTM 溯源 / 2m 采样 / 1737.4km 参考球 / 诚实准入状态', () => {
    expect(meta.sourceUrl).toContain('NAC_DTM/APOLLO15/NAC_DTM_APOLLO15.TIF');
    ...
    expect(typeof meta.ortho.ifScale).toBe('number'); // float32 I/F → u16 归一化比例留痕
  ...
  it('窗内 100% 有效且高程范围与元数据一致（哈德利平原 ~[-2218,-1876]m）', () => {
    const v = readFileSync(path.join(DIR, 'valid.u8'));
    let nonzero = 0;
    ...
    expect(meta.minimumHeightM).toBeGreaterThan(-2400);
  ...
  it('窗外采样 fail-closed（不外推）', () => {
    ...
    const dLon = 1700 / (mPerDeg * Math.cos(26 * Math.PI / 180));
    expect(sampleHeight(26.1324 + dLat, 3.616).valid).toBe(false);
    expect(sampleHeight(26.1324 - dLat, 3.616).valid).toBe(false);
    expect(sampleHeight(26.1324, 3.616 + dLon).valid).toBe(false);
    expect(sampleHeight(26.1324, 3.616 - dLon).valid).toBe(false);
  });

  it('包规范与 RasterTerrainSource 装载语义一致（injectBuffers 全链哈希校验）', async () => {
    ...
    // 着陆点采样与镜像公式一致
    const s = src.sampleHeight(26.1324, 3.6338);
    expect(s.valid).toBe(true);
    expect(Math.abs(s.heightM! - (-1924.7))).toBeLessThanOrEqual(0.5);
    // 窗口边界反解
    const wb = src.windowBounds!;
    expect(wb.latMax).toBeGreaterThan(26.2);
    expect(wb.latMin).toBeLessThan(26.05);
    expect(wb.lonMin).toBeGreaterThan(3.55);
    expect(wb.lonMax).toBeLessThan(3.68);
  });
});

describe('S5-2：lola-l1-hadley-v1 LOLA L1 窗包', () => {
  ...
  it('LOLA↔NAC 独立交叉校验：三锚点差 ≤5m（README 拟合 RMS 1.74m 量级）', () => {
    const cross = lmeta.crosscheckNacDtm;
    expect(cross).toHaveLength(3);
    for (const c of cross) {
      expect(c.nacM).not.toBeNull();
      expect(Math.abs(c.diffM!)).toBeLessThanOrEqual(5);
    }
    // 站点差即 README 声称量级
    expect(Math.abs(cross[0].diffM!)).toBeLessThanOrEqual(2);
  });
  ...
  it('窗口边界反解与采样 fail-closed（镜像公式）', () => {
    ...
    expect(lonMin).toBeGreaterThan(0);
    expect(lonMin).toBeLessThan(0.05); // 不对称窗西缘贴 0° 经线（不环绕）
    expect(latMax).toBeGreaterThan(30.2);
    ...
    const site = sample(26.1324, 3.6338)!;
    expect(Math.abs(site - (-1925.4))).toBeLessThanOrEqual(0.1);
    expect(sample(26.1324, -1.0)).toBeNull(); // 负经度（环绕区）不隐式回卷
    expect(sample(33, 3.6)).toBeNull();
  });
});
