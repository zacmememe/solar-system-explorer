...
底部仪表采用用户选定的 A 经典弧面：左侧常驻时间数字、调速箭头和暂停；中央为关系图或任务弧，当前操作放在图下沿；右侧显示天体名称与当前阶段的主读数。左右仪表轻微内收，保留中央视野。观星看平均半径，下降看实际离地高度，落地看视线方位；准备期显示“准备中”。北向视向、完整仪表、白昼选时和科普资料按需展开，不重复提供“位置”菜单。实现与验证见 [A 方向接入报告](docs/hud-classic-a-17.md)。

中途放弃降落会从当前位置升空，结束后保留同侧观察位置；换落区先沿星球外侧绕行。降落保持当前昼夜，需要白昼时可在底部“观测选项 → 时间倍率 → 切换到所选落区的白昼”主动选时。
